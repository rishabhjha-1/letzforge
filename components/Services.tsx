const services = [
  ...
  },
  {
    ...
    desc: "UX writing, product copy, go-to-market messaging — words designed with the same precision as the interfaces they live inside.",
    tags: ["UX Copy", "GTM", "Content Design"],
  },
  {
    icon: "▣",
    name: "Landing Pages",
    desc: "High-converting, pixel-perfect landing pages built for SaaS launches, campaigns, and product announcements. Fast to ship, designed to sell.",
    tags: ["Conversion", "Web Design", "CRO"],
  },
  {
    icon: "◉",
    name: "SEO Specialist",
    desc: "Technical SEO, content architecture, and on-page optimisation for SaaS and service businesses. We help you get found by the right people at the right time.",
    ...
    <section id="services" className="border-t border-border bg-surface px-12 py-24">
      ...
      <div className="flex flex-col md:flex-row md:justify-between md:items-end gap-8 mb-14">
        ...
        <p className="text-muted text-[0.9rem] leading-[1.7] max-w-[34ch] md:text-right">
          Six focused disciplines. One studio that takes you from concept to a product your users keep coming back to.
        </p>
      </div>

      {/* 3×2 grid */}
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 rounded overflow-hidden border border-border">
        {services.map((s, i) => {
          const col = i % 3;
          ...
          return (
            ...
            >
              <div className="w-11 h-11 flex items-center justify-center rounded-md border border-border bg-surface2 text-xl mb-7">
                {s.icon}
              </div>
              <div
              ...
              <div className="flex flex-wrap gap-1.5 mt-6">
                {s.tags.map((t) => (
                  <span
                    key={t}
                    className="text-accent text-[0.62rem] tracking-[0.1em] uppercase font-bold border border-accentBg px-2 py-0.5 rounded-sm"
                  >
                    {t}
                  </span>
                ))}
